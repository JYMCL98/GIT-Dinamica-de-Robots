...
def calcular():
    global T_modifica,HR_modifica,p_modifica,cantidad_
    ...
    p2 = [p_modifica]
    cantidad_ = len(T_modifica)
    calculos(T_modifica,HR_modifica,p_modifica)
    return T2,HR2,p2,cantidad_
...
def read_file():
    global presion,temp,HumedadR,cantidad_,presion_,temp_,HumedadR_
    file = filedialog.askopenfilename(initialdir ='/',title='Select a file',filetype=(('xlsx files', '*.xlsx*'),('All files', '*.*')))
    #ubicacion['text'] = file

    data = pd.read_excel(file)
    presion = pd.DataFrame(data, columns=['P']).to_numpy() # kPa
    temp = pd.DataFrame(data, columns=['T']).to_numpy() # °C
    HumedadR = pd.DataFrame(data, columns=['HR']).to_numpy() # %

    conv = list(map(float, presion)) # Convertir el arreglo a lista de tipo flotante
    conv_2 = np.array(conv) # convertir a arreglo nuevamente

    conv_3 = list(map(float, temp))
    conv_4 = np.array(conv_3)

    conv_5 = list(map(float, HumedadR))
...
def calculos(T_,HR_,p_):
    ...
    # Botón de cerrar
    boton2=tk.Button(win,text="Cerrar",command=ventana.destroy)
...
def grafica():
    ...
    p = p_modifica
    p *=7.501 # mmHg
    rango_2 = np.arange(0, 55, 5) # Creamos un arreglo de 0 hasta 55, en intervalos de 5, ignora el hasta
    ...
    # Introducir datos manualmente cada 5 °C
...
def B1():
    for i in range(0,cantidad_):
        calculos(temp_[i],HumedadR_[i],presion_[i])
        plt.scatter(temp_[i],W,color='red',linewidth=3)
        plt.plot([temp_[i],temp_[i]],[W,0],'b-',label='Tw',linewidth=3)
        plt.plot([temp_[i],100],[W,W],'m-',label='W',linewidth=3)
        plt.plot([temp_[i],Tpr],[W,W],'g-',label='W',linewidth=3) # Tpr (izquierda)
        plt.plot([Tpr,Tpr],[W,0],'m-',label='W',linewidth=3) # Tpr (baja)
        plt.plot([temp_[i],Tw],[W,Ws_1],'g-',label='W',linewidth=3) # Tw (sube) y_punto rocío = Es la W al 100% de humedad
        plt.plot([Tw,Tw],[0,Ws_1],'k-',label='Tw',linewidth=3) # Tw (baja) y_punto rocío = Es la W al 100% de humedad
        plt.plot([temp_[i],temp_[i]],[Ws,W],'y-',label='Ws',linewidth=3) # Ws (sube) 
        plt.plot([temp_[i],100],[Ws,Ws],'b-',label='Ws',linewidth=3) # Ws (derecha) 
        plt.plot([H,0],[0,Ws],'g-',label='h',linewidth=3) # h
    grafica()
...
e1=tk.Label(ventana,text="Temperatura, [°C]: ",bg="red",fg="white")
e1.pack(padx=5,pady=5,ipadx=5,ipady=5)
entrada1=tk.Entry(ventana)
entrada1.pack(fill=tk.X,padx=5,pady=5,ipadx=5,ipady=5)

e2=tk.Label(ventana,text="Humedad Relativa, [%]: ",bg="#0B2769",fg="white")
...
# Botón que manda a llamar la función gráfica
grafica=tk.Button(ventana,text="Graficar",fg="blue",command=grafica)
grafica.pack(side=tk.TOP)

e5=tk.Label(ventana,text="Abrir archivo de Excel",bg="#0B2769",fg="white")
e5.pack(padx=5,pady=5,ipadx=5,ipady=5)
boton_excel = tk.Button(ventana,text="Open File",fg="blue",command=read_file)
boton_excel.pack(side=tk.TOP)

boton_excel_grafica = tk.Button(ventana,text="Plot",fg="blue",command=B1)
boton_excel_grafica.pack(side=tk.TOP)
...
ubicacion = Label(frame2, fg= 'white', bg='#1F0202', text= 'Location: ', font= ('Times New Roman',10,'bold') )
